package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_720:IFigurePartSet;
      ...
      private const const_1779:int = 13421772;
      
      private const const_1780:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_612 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1760 = Math.max(this.var_1760,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_612 != null && !this.var_1758)
            {
               _loc3_ = this.var_612;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_720)
            {
               switch(this.var_720.clubLevel)
               {
                  case HabboClubLevelEnum.const_49:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_61:
                     _loc2_.style = HabboIconType.const_773;
                     break;
                  case HabboClubLevelEnum.const_53:
                     _loc2_.style = HabboIconType.const_620;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_311[_loc6_] + "_" + FigureData.const_139;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_596 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1759)
               {
                  ...
                  this.var_1759 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_596.width,this.var_596.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_596.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_596.y;
               if(this.var_1758 && _loc2_.colorLayerIndex > 0)
               {
