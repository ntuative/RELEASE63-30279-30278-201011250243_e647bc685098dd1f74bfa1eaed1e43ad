package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private static var var_1265:String = "%";
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1265);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1265,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
         ...
         if(param2 != null)
         {
            ...
            if(!this.var_1585)
            {
               if("false" == this.var_164.getKey("use.default.localizations","false"))
               {
                  ...
                  events.addEventListener(LocalizationEvent.const_155,this.onLocalizationLoaded);
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_155,this.onLocalizationLoaded);
         if(this.var_164.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_164.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_155,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_155,this.onLocalizationOverrideLoaded);
         unlock();
      }
