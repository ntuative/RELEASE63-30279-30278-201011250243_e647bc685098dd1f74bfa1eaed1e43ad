package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2146 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2145 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2147 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1583 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_195 != null && this.var_195.running)
         {
            this.var_195.reset();
            return false;
         }
         if(this.var_1014 == 0)
         {
            this.var_1014 = getTimer();
            return false;
         }
         if(this.var_1583 > 0 && this.var_1582 >= this.var_1583)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1014;
         if(_loc1_ > this.var_2145)
         {
            ++this.var_1582;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1264 = _loc2_.imageUrl;
         this.var_2148 = _loc2_.clickUrl;
         this.var_1014 = getTimer();
         if(this.var_1264 != "")
         {
            _loc3_ = this.var_1264;
            ...
            {
               this.var_195 = new Timer(this.var_2147,1);
               this.var_195.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_195.running)
            {
               this.var_195.reset();
            }
            this.var_195.start();
            _loc4_ = new URLRequest(this.var_1264);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_453,0,0,param1.bitmapData,this.var_2148));
            this.var_1014 = getTimer();
         }
         else
         {
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1263 = _loc2_.imageUrl;
         this.var_1584 = _loc2_.clickUrl;
         if(this.var_1263 != "" && this.var_1584 != "")
         {
            _loc3_ = this.var_1263;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1263);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_523,0,0,_loc4_,this.var_1584,this.var_1012,this.var_1013));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1012 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1013 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
