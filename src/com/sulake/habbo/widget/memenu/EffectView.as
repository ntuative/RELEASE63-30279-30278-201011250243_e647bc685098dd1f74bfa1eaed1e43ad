package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_154:IWidgetAvatarEffect;
      
      private var var_773:IWindow;
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_154.isActive)
            {
               _loc2_ = this.var_154.secondsLeft / Number(this.var_154.duration);
               this.var_773.width = _loc2_ * this.var_485;
            }
            else
            {
               this.var_773.width = 0;
               this.var_175.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_771.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_771.text;
            if(this.var_154.isActive)
            {
               _loc2_ = this.var_154.secondsLeft;
               ...
               this.var_771.text = _loc1_;
            }
            else
            {
               this.var_771.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_154.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_154.isActive)
         {
         ...
         else
         {
            ...
            this.var_772 = this._window.findChildByName("effect_hilite");
            this.var_772.visible = false;
         }
         this.var_771 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_773 = this._window.findChildByName("loader_bar");
         if(this.var_773 != null)
         {
            this.var_485 = this.var_773.width;
            this.var_175.start();
            this.onUpdate();
         }
         if(this.var_154.icon)
         {
            this.setElementBitmap("effect_icon",this.var_154.icon);
         }
      }
